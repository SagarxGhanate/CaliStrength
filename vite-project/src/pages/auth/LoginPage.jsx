  ...
  function handleLogin(isGoogle = false) {
    ...
    if (raw) {
      try {
        ...
        if (isGoogle) {
          if (!data.profile) data.profile = { name: 'User', email: '', avatar: '' }
          data.profile.avatar = 'https://ui-avatars.com/api/?name=User&background=ec5b13&color=fff&rounded=true&bold=true'
  ...
  return (
    <div className={styles.authWrapper}>
      <div className={styles.authCard}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleLogin(false) }}>
          ...
          <button type="submit" className={styles.btnBrand}>Sign In</button>
        </form>

        <div className={styles.divider}>or</div>

        <button className={styles.btnGoogle} onClick={() => handleLogin(true)}>
